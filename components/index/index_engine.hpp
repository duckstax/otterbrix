...
namespace components::index {
    ...
    public:
        ...
        auto find(id_index id) -> index_t::pointer ;
        auto find(const keys_base_storage_t & query) -> index_t::pointer;
    ...
    auto make_index(index_engine_ptr& ptr, const keys_base_storage_t & keys, Args&&... args) -> uint32_t {
        return ptr->add_index(keys, core::pmr::make_unique<Target>(ptr->resource(),ptr->resource(), keys, std::forward<Args>(args)...));
    }

...
namespace services::collection::planner::impl {
    ...
    components::collection::operators::operator_ptr create_plan_sort(const context_storage_t& context,
        ...
        auto sort = boost::intrusive_ptr(
            new components::collection::operators::operator_sort_t(context.at(node->collection_full_name())));
        std::for_each(node->expressions().begin(),
                      ...
                      [&sort](const components::expressions::expression_ptr& expr) {
                          ...
                          sort->add(sort_expr->key().as_string(),
                                    components::collection::operators::operator_sort_t::order(sort_expr->order()));
                      });
        return sort;
    }

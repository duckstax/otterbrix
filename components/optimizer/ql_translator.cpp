...
namespace components::optimizer {
    ...
    auto translator_aggregate(ql::aggregate_statement* aggregate) -> logical_plan::node_ptr {
        ...
        auto node = new logical_plan::node_aggregate_t;
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            switch (type) {
                case operator_type::match:
                    node->append_child(logical_plan::make_node_match(aggregate->get_operator<ql::aggregate::match_t>(i)));
                    break;
                case operator_type::group:
                    node->append_child(logical_plan::make_node_group(aggregate->get_operator<ql::aggregate::group_t>(i)));
                    break;
                case operator_type::sort:
                    node->append_child(logical_plan::make_node_sort(aggregate->get_operator<ql::aggregate::sort_t>(i)));
                    break;
                default:
                    break;
            }
    ...
    auto ql_translator(ql::ql_statement_t* statement) -> logical_plan::node_ptr {
        ...
        switch (statement->type_) {
            ...
            case statement_type::aggregate:
                return translator_aggregate(static_cast<ql::aggregate_statement*>(statement));
            default:

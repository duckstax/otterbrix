...
TEST_CASE("parser::select_from_where") {
    ...
    TEST_SIMPLE_SELECT("select * from schema.table;",
                       R"_($aggregate: {})_",
                       vec());
    ...
    TEST_SIMPLE_SELECT("select * from schema.table where number = 10 and name = 'doc 10' and count = 2;",
                       R"_($aggregate: {$match: {$and: ["number": {$eq: #0}, "name": {$eq: #1}, "count": {$eq: #2}]}})_",
                       vec({v(10l), v(std::string("doc 10")), v(2l)}));

    TEST_SIMPLE_SELECT("select * from schema.table where number = 10 or name = 'doc 10' or count = 2;",
                       R"_($aggregate: {$match: {$or: ["number": {$eq: #0}, "name": {$eq: #1}, "count": {$eq: #2}]}})_",
                       vec({v(10l), v(std::string("doc 10")), v(2l)}));

    TEST_SIMPLE_SELECT("select * from schema.table where number = 10 and name = 'doc 10' or count = 2;",
    ...
    TEST_SIMPLE_SELECT("select * from schema.table where ((number = 10 and name = 'doc 10') or count = 2) and "

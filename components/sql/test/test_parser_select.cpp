...
TEST_CASE("parser::select_without_from") {
    ...
    TEST_SELECT_WITHOUT_FROM(R"_(select 10 number;)_", R"_($aggregate: {$group: {number: #0}})_", vec({v(10l)}));

    TEST_SELECT_WITHOUT_FROM(R"_(select 'title' as title;)_",
                             R"_($aggregate: {$group: {title: #0}})_",
                             vec({v(std::string("title"))}));

    TEST_SELECT_WITHOUT_FROM(R"_(select 10;)_", R"_($aggregate: {$group: {10: #0}})_", vec({v(10l)}));

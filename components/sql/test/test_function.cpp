...
TEST_CASE("sql::function") {
    ...
    SECTION("Mutable operations") {
        TEST_SIMPLE_FUNCTION(
            R"_(INSERT INTO col1 (column1, column2) SELECT * FROM sql_func('db.col', "select something else");)_",
            R"_($insert: {$aggregate: {$function: {name: {"sql_func"}, args: {#0, #1}}}})_",
            vec({new_value("db.col"), new_value("select something else")}));

        TEST_SIMPLE_FUNCTION(
            R"_(UPDATE col1 SET column1 = col.column1 FROM sql_func('db.col', "select something else") AS col WHERE col1.id = col.id;)_",
            R"_($update: {$upsert: 0, $match: {"id": {$eq: "id"}}, $limit: -1, $function: {name: {"sql_func"}, args: {#0, #1}}})_",
            vec({new_value("db.col"), new_value("select something else")}));

        TEST_SIMPLE_FUNCTION(
            R"_(DELETE FROM col1 USING sql_func('db.col', "select something else") AS col WHERE col1.id = col.id;)_",
            R"_($delete: {$match: {"id": {$eq: "id"}}, $limit: -1, $function: {name: {"sql_func"}, args: {#0, #1}}})_",
            vec({new_value("db.col"), new_value("select something else")}));
    }
}
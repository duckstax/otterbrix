...
TEST_CASE("parser::delete_from_where") {
    ...
    TEST_SIMPLE_DELETE("delete from schema.table where not (number = 10) and not(name = 'doc 10' or count = 2);",
...
TEST_CASE("parser::delete errors") {
    ...
    TEST_ERROR_DELETE("delete from schema.table where number == 10 group by name;",
                      sql::parse_error::syntax_error, "group", 44);

    TEST_ERROR_DELETE("delete from schema.table where number == 10 order by name;",
                      sql::parse_error::syntax_error, "order", 44);

    TEST_ERROR_DELETE("delete from schema. where number == 10;",
                      sql::parse_error::syntax_error, " ", 19);

    TEST_ERROR_DELETE("delete from .table where number == 10;",
                      sql::parse_error::syntax_error, ".", 12);

    TEST_ERROR_DELETE("delete from schema.table where number == 10 name = 'doc 10';",
                      sql::parse_error::syntax_error, "name", 44);

    TEST_ERROR_DELETE("delete from schema.table where number == 10 and and = 'doc 10';",
                      sql::parse_error::syntax_error, "and", 48);
}

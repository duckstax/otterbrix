...
namespace components::sql::table::impl {
    ...
    parse_drop(std::pmr::memory_resource*, std::string_view query, ql::variant_statement_t& statement) {
        static mask_t mask_with_db({mask_element_t(token_type::bare_word, "drop"),
        ...
        static mask_t mask_without_db({mask_element_t(token_type::bare_word, "drop"),
                                       mask_element_t(token_type::whitespace, ""),
                                       mask_element_t(token_type::bare_word, "table"),
                                       mask_element_t(token_type::whitespace, ""),
                                       mask_element_t::create_value_mask_element(),
        ...
        if (mask_with_db.match(lexer)) {
            statement =
                components::ql::drop_collection_t{mask_with_db.cap(index_name_db), mask_with_db.cap(index_name_table)};
            return true;
        }
        lexer.restore();
        if (mask_without_db.match(lexer)) {
            statement =
                components::ql::drop_collection_t{database_name_t(), mask_without_db.cap(index_name_table_without_db)};
            return true;
        }
        return false;
    }

#include "parser_table_create.hpp"
...
namespace components::sql::table::impl {
    ...
    parse_create(std::pmr::memory_resource*, std::string_view query, ql::variant_statement_t& statement) {
        static mask_t mask_with_db({mask_element_t(token_type::bare_word, "create"),
        ...
        static mask_t mask_without_db({mask_element_t(token_type::bare_word, "create"),
                                       mask_element_t(token_type::whitespace, ""),
                                       mask_element_t(token_type::bare_word, "table"),
                                       mask_element_t(token_type::whitespace, ""),
                                       mask_element_t::create_value_mask_element(),
        ...
        if (mask_with_db.match(lexer)) {
            statement = components::ql::create_collection_t{mask_with_db.cap(index_name_db),
                                                            mask_with_db.cap(index_name_table)};
            return true;
        }
        lexer.restore();
        if (mask_without_db.match(lexer)) {
            statement = components::ql::create_collection_t{database_name_t(),

...
namespace components::sql {
    ...
    std::size_t token_t::size() const { return end - begin; }

    std::string_view token_t::value() const { return std::string_view{begin, size()}; }

    bool operator==(const token_t& token1, const token_t& token2) {
        return token1.type == token2.type &&
               (token1.type != token_type::bare_word || token_clean_value(token1) == token_clean_value(token2));
    }

    bool operator<(const token_t& token1, const token_t& token2) {
        return token1.type < token2.type ||
               (token1.type == token2.type && token_clean_value(token1) < token_clean_value(token2));
    }

    bool is_token_significant(const token_t& token) {
        return token.type != token_type::whitespace && token.type != token_type::comment;
    }

    bool is_token_error(const token_t& token) { return token.type >= token_type::error; }

    bool is_token_end(const token_t& token) { return token.type == token_type::end_query; }

    bool is_token_end_query(const token_t& token) {
        return token.type == token_type::end_query || token.type == token_type::semicolon;
    }
    ...
    bool is_token_field_name(const token_t& token) {
        return (token.type == token_type::bare_word || token.type == token_type::quoted_identifier) &&
               !is_token_bool_value(token);
    }

    bool is_token_field_value(const token_t& token) {
        return token.type == token_type::number_literal || token.type == token_type::string_literal ||
               is_token_bool_value(token);
    }

    std::string_view token_name(token_type type) { return magic_enum::enum_name(type); }

    std::string_view token_name(const token_t& token) { return token_name(token.type); }

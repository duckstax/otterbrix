...
TEST_CASE("sql_new::create_database") {
    ...
    components::ql::tape_wrapper tape(&resource);

    SECTION("create") {
        auto create = raw_parser("CREATE DATABASE db_name")->lst.front().data;
        auto node = transformer.transform(transform::pg_cell_to_node_cast(create), tape.tape());
...
TEST_CASE("sql_new::drop_database") {
    ...
    auto node = transformer.transform(transform::pg_cell_to_node_cast(drop), tape.tape());

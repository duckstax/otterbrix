...
void _precondition_failed(const char *cond, const char *fn, const char *file, int line) {
    throw std::invalid_argument(log("FAILED PRECONDITION: `%s` not true when calling %s (at %s line %d)", cond, fn, file, line));
}

void _postcondition_failed(const char *cond, const char *fn, const char *file, int line) {
    throw assertion_failure(log("FAILED POSTCONDITION: `%s` not true at end of %s (at %s line %d)", cond, fn, file, line));
}

...
WasmResult mock_wasm_context_t::log(uint32_t log_level, string_view message) {
    ...
    if (id_ == "plugin_id0") {
        //REQUIRE(message == "[/ottergon/components/wasm_runner/tests/log_wasm/main.cpp:17]::onStart() Hello, World!");
    }

    if (id_ == "plugin_id1") {
        //REQUIRE(message == "[/ottergon/components/wasm_runner/tests/log_quickjs_wasm/main.cpp:159]::js_log() Hello, World!");
    }

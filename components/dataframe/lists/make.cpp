...
namespace components::dataframe::lists {
    std::unique_ptr<column::column_t> make_lists_column(
        ...
        assertion_exception_msg((num_rows == 0 && offsets_column->size() == 0) || num_rows == offsets_column->size() - 1,"Invalid offsets column size for lists column.");

...
namespace components::dataframe::column {
    ...
    struct scalar_empty_like_functor_impl {
        std::unique_ptr<column::column_t> operator()(std::pmr::memory_resource*resource,scalar::scalar_t const& input){
    ...
    std::unique_ptr<table::table_t> empty_like(std::pmr::memory_resource* resource,table::table_view const& input_table){
    ...
    std::unique_ptr<column::column_t> empty_like(std::pmr::memory_resource* resource, column::column_view const& input) {
        ...
        return std::make_unique<column::column_t>(resource,input.type(), 0, core::buffer(resource), core::buffer(resource), 0, std::move(children));
    }

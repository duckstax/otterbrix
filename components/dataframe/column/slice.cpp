...
namespace components::dataframe::column {
    ...
    std::vector<column::column_view> slice(std::pmr::memory_resource*resource,column::column_view const& input, core::span<size_type const> indices) {
        assertion_exception_msg(indices.size() % 2 == 0, "indices size must be even");

        if (indices.empty())
            return {};

        auto iter_start = make_counting_transform_iterator(0, [offset = input.offset(), &indices](size_type index) { return indices[index] + offset; });
        auto iter_finish = make_counting_transform_iterator(indices.size(), [offset = input.offset(), &indices](size_type index) { return indices[index] + offset; });
        auto null_counts = dataframe::detail::segmented_null_count(resource,input.null_mask(), {iter_start.base().operator->(), iter_finish.base().operator->()});
    ...
    std::unique_ptr<column_t> copy_slice(
        ...
        auto offsets_column = std::make_unique<column_t>(resource,slice(strings.offsets(), {offsets_offset, offsets_offset + strings_count + 1}).front());
        auto const chars_offset = offsets_offset == 0 ? 0 : dataframe::detail::get_value<int32_t>(offsets_column->view(), 0);
        if (chars_offset > 0) {
        ...
        auto chars_column = std::make_unique<column_t>(resource, slice(strings.chars(), {chars_offset, chars_offset + data_size}).front());

        // slice the null mask
        auto null_mask = dataframe::detail::copy_bitmask(resource,strings.null_mask(), offsets_offset, offsets_offset + strings_count);

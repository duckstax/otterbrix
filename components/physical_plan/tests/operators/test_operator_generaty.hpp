...
struct context_t final {
    context_t(log_t& log)
        ...
        , collection_([this](auto& log) {
            ...
            return std::unique_ptr<context_collection_t, actor_zeta::deleter>(collection,
                                                                              actor_zeta::deleter(resource_));
        }(log)) {}
    ...
    std::unique_ptr<context_collection_t, actor_zeta::deleter> collection_;
};
...
inline context_ptr make_context(log_t& log) { return std::make_unique<context_t>(log); }

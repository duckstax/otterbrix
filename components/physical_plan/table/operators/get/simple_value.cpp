...
namespace components::table::operators::get {
    ...
    types::logical_value_t simple_value_t::get_value_impl(const std::pmr::vector<types::logical_value_t>& row) {
        auto it = std::find_if(row.begin(), row.end(), [&](const types::logical_value_t& v) {

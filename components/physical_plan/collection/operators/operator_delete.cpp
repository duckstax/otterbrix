    ...
    void operator_delete::on_execute_impl(pipeline::context_t* pipeline_context) {
        ...
        if (left_ && left_->output() && right_ && right_->output()) {
            ...
            for (auto& doc_left : left_->output()->documents()) {
                for (auto& doc_right : right_->output()->documents()) {
                    if (match_predicate_->check(doc_left, doc_right, &pipeline_context->parameters)) {
                        const auto id = document::get_document_id(doc_left);
                        auto it = context_->document_storage().find(id);
                        if (it != context_->document_storage().end()) {
                            context_->document_storage().erase(it);
                            modified_->append(id);
                            components::base::operators::update_types_from_document(modified_->updated_types_map(),
                                                                                    doc_left);
            ...
            for (const auto& document : left_->output()->documents()) {
                const auto id = document::get_document_id(document);
                auto it = context_->document_storage().find(id);
                if (it != context_->document_storage().end()) {
                    context_->document_storage().erase(it);
                    modified_->append(id);

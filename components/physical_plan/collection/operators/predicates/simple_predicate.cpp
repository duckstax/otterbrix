...
namespace services::collection::operators::predicates {
    ...
    simple_predicate::simple_predicate(std::function<bool(const components::document::document_ptr&,
                                                          const components::document::document_ptr&,
                                                          const components::logical_plan::storage_parameters*)> func)
        : func_(std::move(func)) {}

    simple_predicate::simple_predicate(std::vector<predicate_ptr>&& nested,
    ...
    predicate_ptr create_simple_predicate(const components::expressions::compare_expression_ptr& expr) {
        ...
        switch (expr->type()) {
            ...
            case compare_type::eq:
                return {new simple_predicate([&expr](const components::document::document_ptr& document_left,
                                                     const components::document::document_ptr& document_right,
                                                     const components::logical_plan::storage_parameters* parameters) {
                    if (expr->key_right().is_null()) {
                        ...
                        if (it == parameters->parameters.end()) {
                            ...
                            return document_left->compare(expr->key_left().as_string(), it->second) ==
                                   components::types::compare_t::equals;
                        }
                    } else {
                        auto comp =
                            document_right
                        ...
                        return comp == components::types::compare_t::equals;
                    }
                })};
                ...
                return {new simple_predicate([&expr](const components::document::document_ptr& document_left,
                    ...
                    if (expr->key_right().is_null()) {
                        ...
                        if (it == parameters->parameters.end()) {
                            ...
                            return document_left->compare(expr->key_left().as_string(), it->second) ==
                                   components::types::compare_t::more;
                        }
                    } else {
                        auto comp =
                            document_right
                                ...
                                : document_left->compare(expr->key_left().as_string(),
                                                         document_left->get_value(expr->key_right().as_string()));
                        return comp == components::types::compare_t::more;
                    }
                })};
                ...
                return {new simple_predicate([&expr](const components::document::document_ptr& document_left,
                    ...
                    if (expr->key_right().is_null()) {
                        ...
                        if (it == parameters->parameters.end()) {
                            ...
                            return comp == components::types::compare_t::equals ||
                                   comp == components::types::compare_t::more;
                        }
                    } else {
                        auto comp =
                            document_right
                    ...
                    if (expr->key_right().is_null()) {
                        ...
                        auto comp =
                            document_right
                                ...
                                : document_left->compare(expr->key_left().as_string(),
                                                         document_left->get_value(expr->key_right().as_string()));
                        return comp == components::types::compare_t::less;
                    }
                })};
                ...
                return {new simple_predicate([&expr](const components::document::document_ptr& document_left,
                    ...
                    if (expr->key_right().is_null()) {
                        ...
                        if (it == parameters->parameters.end()) {
                            ...
                            auto comp = document_left->compare(expr->key_left().as_string(), it->second);
                            return comp == components::types::compare_t::equals ||
                                   comp == components::types::compare_t::less;
                        }
                    } else {
                        auto comp =
                            document_right
            ...
            case compare_type::all_true:
                return {new simple_predicate([](const components::document::document_ptr&,
                                                const components::document::document_ptr&,
                                                const components::logical_plan::storage_parameters*) { return true; })};
            case compare_type::all_false:
                return {
                    new simple_predicate([](const components::document::document_ptr&,
                                            const components::document::document_ptr&,

...
namespace services::base::operators {
    ...
    operator_add_index::operator_add_index(collection::context_collection_t* context,
    ...
    void operator_add_index::on_execute_impl(components::pipeline::context_t* pipeline_context) {
        ...
        switch (index_node_->type()) {
            case components::logical_plan::index_type::single: {
                ...
                collection::sessions::make_session(
                    context_->sessions(),
                    pipeline_context->session,
                    index_node_->name(),
                    collection::sessions::create_index_t{pipeline_context->current_message_sender, id_index});

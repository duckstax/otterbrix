...
namespace components::catalog {
    ...
    void metadata_diff::update_description(const std::pmr::string& desc) { new_desc_.emplace(desc); }

    void metadata_diff::use_schema_diff(schema_diff&& diff) { schema_diff_ = std::move(diff); }

    bool metadata_diff::has_changes() const { return new_desc_ || schema_diff_.has_changes(); }
    ...
    table_metadata metadata_diff::apply(const table_metadata& base_metadata) const {
        ...
        return table_metadata(resource_,
                              ...
                              new_desc_.value_or(base_metadata.description()));
    }
} // namespace components::catalog

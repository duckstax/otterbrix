...
namespace components::catalog {
    table_metadata::table_metadata(std::pmr::memory_resource* resource,
        ...
        : schema_struct(std::move(schema))
        , table_description(description, resource)
        , last_updated_ms_(std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::system_clock::now().time_since_epoch()))
        , next_column_id_(schema_struct.highest_field_id() + 1) {}

    const std::pmr::string& table_metadata::description() const { return table_description; }
    ...
    const schema& table_metadata::current_schema() const { return schema_struct; }
} // namespace components::catalog

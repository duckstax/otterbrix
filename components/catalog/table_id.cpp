...
namespace components::catalog {
    ...
    table_id::table_id(std::pmr::memory_resource* resource, std::pmr::vector<std::pmr::string> full_name)
        : namespace_parts(std::move(full_name), resource)
        , name(namespace_parts.back())
        , resource(resource) {
        namespace_parts.erase(namespace_parts.cend() - 1);
    }

    table_id::table_id(std::pmr::memory_resource* resource, table_namespace_t ns, std::pmr::string name)
        : namespace_parts(std::move(ns), resource)
        , name(std::move(name), resource)
        , resource(resource) {}

    table_id::table_id(std::pmr::memory_resource* resource, const collection_full_name_t& full_name)
        : namespace_parts(resource)
        , name(full_name.collection)
        , resource(resource) {
        bool has_schema = !full_name.schema.empty();
        bool has_uid = !full_name.unique_identifier.empty();

        if (has_uid) {
            namespace_parts.emplace_back(full_name.unique_identifier.c_str());
        }

        if (has_schema || has_uid) {
            namespace_parts.emplace_back(full_name.schema.c_str());
        }

        namespace_parts.emplace_back(full_name.database.c_str());
    }

    bool table_id::operator==(const table_id& other) const {
        return namespace_parts == other.namespace_parts && name == other.name;
    }
    ...
    const table_namespace_t& table_id::get_namespace() const { return namespace_parts; }

    const std::pmr::string& table_id::table_name() const { return name; }

    collection_full_name_t table_id::collection_full_name() const {
        switch (namespace_parts.size()) {
    ...
    std::pmr::string table_id::to_pmr_string() const {
        ...
        for (size_t i = 0; i < namespace_parts.size(); ++i) {
            if (i > 0)
                oss << ".";
            oss << namespace_parts[i];
        }
        oss << "." << name;
        return std::pmr::string(oss.str(), resource);
    }

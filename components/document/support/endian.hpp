...
CONST inline uint16_t swap_little(uint16_t n) noexcept { return static_cast<uint16_t>(little_enc16(n)); }
CONST inline uint16_t swap_big(uint16_t n)    noexcept { return static_cast<uint16_t>(enc16(n)); }
...
inline void swap_big(uint16_t &n)    noexcept { n = static_cast<uint16_t>(enc16(n)); }
...
{
public:
    endian_unaligned() noexcept : endian_unaligned(0) {}

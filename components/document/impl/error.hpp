...
namespace components::document::impl {
    ...
    template<typename T>
    struct document_result : public document_result_base<T> {
        document_result() noexcept
            : document_result_base<T>() {}

        document_result(T&& value) noexcept
            : document_result_base<T>(std::forward<T>(value)) {}

        document_result(error_code error) noexcept
            : document_result_base<T>(error) {}

        document_result(T&& value, error_code error) noexcept
            : document_result_base<T>(std::forward<T>(value), error) {}

        void tie(T& value, error_code& error) && noexcept {
            std::forward<document_result_base<T>>(*this).tie(value, error);
        }

        error_code get(T& value) && noexcept { return std::forward<document_result_base<T>>(*this).get(value); }

        error_code error() const noexcept { return this->second; }

        T& value() & noexcept(false) { return document_result_base<T>::value(); }

        T&& value() && noexcept(false) { return std::forward<document_result_base<T>>(*this).value(); }

        T&& take_value() && noexcept(false) { return std::forward<document_result_base<T>>(*this).take_value(); }

        operator T&&() && noexcept(false) { return std::forward<document_result_base<T>>(*this).take_value(); }

        const T& value_unsafe() const& noexcept { return document_result_base<T>::value_unsafe(); }

        T&& value_unsafe() && noexcept { return std::forward<document_result_base<T>>(*this).value_unsafe(); }

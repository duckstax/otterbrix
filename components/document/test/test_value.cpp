...
TEST_CASE("value_t") {
    ...
    SECTION("Var int read") {
        ...
        while (d <= static_cast<double>(UINT64_MAX)) {
            ...
            REQUIRE(get_uvar_int(slice_t(buf, sizeof(buf)), &result) == nBytes);
            REQUIRE(result == n);
            REQUIRE(get_uvar_int(slice_t(buf, nBytes), &result) == nBytes);
            REQUIRE(result == n);
            REQUIRE(get_uvar_int(slice_t(buf, nBytes - 1), &result) == 0);
            d = std::max(d, 1.0) * 1.5;
        }
        memset(buf, 0x88, sizeof(buf));
        REQUIRE(get_uvar_int(slice_t(buf, sizeof(buf)), &result) == 0);
    }
    ...
    SECTION("Var int32 read") {
        ...
        while (d <= static_cast<double>(UINT64_MAX)) {
            ...
            uint32_t result;
            if (n <= UINT32_MAX) {
                REQUIRE(get_uvar_int32(slice_t(buf, sizeof(buf)), &result) == nBytes);
                REQUIRE(result == n);
                REQUIRE(get_uvar_int32(slice_t(buf, nBytes), &result) == nBytes);
                REQUIRE(result == n);
                REQUIRE(get_uvar_int32(slice_t(buf, nBytes - 1), &result) == 0);
            } else {
                REQUIRE(get_uvar_int32(slice_t(buf, sizeof(buf)), &result) == 0);
            }
            d = std::max(d, 1.0) * 1.5;
        }
    }

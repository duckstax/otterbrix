...
namespace document::impl::internal {
    ...
    bool value_slot_t::empty() const { return _pointer == 0; }

    const value_t* value_slot_t::as_value() const { return is_pointer() ? pointer() : inline_pointer(); }

    value_slot_t::operator bool() const { return !empty(); }
    ...
    void value_slot_t::set(nullptr_t) { set_inline(tag_special, special_value_null); }

    void value_slot_t::set(bool b) { set_inline(tag_special, b ? special_value_true : special_value_false); }

    void value_slot_t::set(int32_t i) { set_int(i); }

    void value_slot_t::set(uint32_t i) { set_int(i); }

    void value_slot_t::set(int64_t i) { set_int(i); }

    void value_slot_t::set(uint64_t i) { set_int(i); }
    ...
    void value_slot_t::set(std::string_view s) { set_string_or_data(tag_string, s); }
    ...
    bool value_slot_t::is_pointer() const noexcept { return _inline._tag != inline_tag; }

    bool value_slot_t::is_inline() const noexcept { return !is_pointer(); }

    const value_t* value_slot_t::pointer() const noexcept { return reinterpret_cast<const value_t*>(_pointer); }

    const value_t* value_slot_t::as_pointer() const noexcept { return (is_pointer() ? pointer() : nullptr); }
    ...
    void value_slot_t::copy_value(copy_flags flags) {
        ...
        if (value && ((flags & copy_immutables) || value->is_mutable())) {
            ...
            switch (value->tag()) {
                case tag_array: {
                    auto copy_value =
                        array_t::new_array(reinterpret_cast<array_t*>(const_cast<value_t*>(value))).detach();
                    if (recurse) {

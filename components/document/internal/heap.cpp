...
#pragma GCC diagnostic pop
    ...
    tags heap_value_t::tag() const { return tags(header_ >> 4); }
    ...
    heap_value_t* heap_value_t::create(nullptr_t) { return new (0) heap_value_t(tag_special, special_value_null); }

    heap_value_t* heap_value_t::create(bool b) {
        return new (0) heap_value_t(tag_special, b ? special_value_true : special_value_false);
    }

    heap_value_t* heap_value_t::create(int i) { return create_int(i, false); }

    heap_value_t* heap_value_t::create(unsigned i) { return create_int(i, true); }

    heap_value_t* heap_value_t::create(int64_t i) { return create_int(i, false); }

    heap_value_t* heap_value_t::create(uint64_t i) { return create_int(i, true); }
    ...
    heap_value_t* heap_value_t::create_int(INT i, bool is_unsigned) {
        if (i < 2048 && (is_unsigned || -i < 2048)) {
            ...
            return create(tag_int,
                          int(size - 1) | (is_unsigned ? 0x08 : 0),
    ...
    heap_value_t* heap_value_t::create(std::string_view s) { return create_str(internal::tag_string, s); }
    ...
    const value_t* heap_value_t::as_value(heap_value_t* v) { return v ? v->as_value() : nullptr; }

    const value_t* heap_value_t::as_value() const { return reinterpret_cast<const value_t*>(&header_); }

    bool heap_value_t::is_heap_value(const value_t* v) { return (size_t(v) & 1) != 0; }
    ...
    static bool is_hardwired_value(const value_t* v) {
        return v == value_t::null_value || v == value_t::undefined_value || v == value_t::true_value ||
               v == value_t::false_value || v == array_t::empty_array() || v == dict_t::empty_dict();
    }
    ...
    void* heap_value_t::operator new(size_t size) { return ::operator new(size); }

    void heap_value_t::operator delete(void* ptr) { ::operator delete(ptr); }

    void heap_value_t::operator delete(void* ptr, size_t) { ::operator delete(ptr); }
    ...
    retained_t<heap_collection_t> heap_collection_t::mutable_copy(const value_t* v, tags if_type) {
        ...
        switch (if_type) {
            case tag_array:
                return reinterpret_cast<const array_t*>(v)->mutable_copy();
            case tag_dict:
                return reinterpret_cast<const dict_t*>(v)->mutable_copy();
            default:
                return nullptr;
        }
    }

    bool heap_collection_t::is_changed() const { return changed_; }

    heap_collection_t::heap_collection_t(tags tag)
        : heap_value_t(tag, 0)
        , changed_(false) {}

    void heap_collection_t::set_changed(bool c) { changed_ = c; }

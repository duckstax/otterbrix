...
heap_dict_t::iterator::iterator(const mutable_dict_t *dict) noexcept
    : iterator((heap_dict_t*)heap_collection_t::as_heap_value(dict))
{}
...
mutable_dict_t *heap_dict_t::as_mutable_dict(heap_dict_t *a) {
    return (mutable_dict_t*)as_value(a);
}

mutable_dict_t *heap_dict_t::as_mutable_dict() const {
    return (mutable_dict_t*)as_value();
}
...
key_t heap_dict_t::_allocate_key(key_t key) {
    if (key.shared())
        return key;
    alloc_slice_t alloced_key(key.as_string());
...
mutable_array_t *heap_dict_t::get_mutable_array(slice_t key) {
    return (mutable_array_t*)as_value(get_mutable(key, tag_array));
}

mutable_dict_t *heap_dict_t::get_mutable_dict(slice_t key) {
    return (mutable_dict_t*)as_value(get_mutable(key, tag_dict));
}

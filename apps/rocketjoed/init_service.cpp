...
void init_service(goblin_engineer::dynamic_environment&env,goblin_engineer::dynamic_config&cfg) {

        auto* http = env.add_manager_service<rocketjoe::network::server>();

        auto python = goblin_engineer::make_service<rocketjoe::services::python_vm>(http,cfg);

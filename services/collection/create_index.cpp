...
namespace services::collection {
    ...
    void collection_t::create_index(const session_id_t& session, create_index_t& index) {
        ...
        if (dropped_) {
            actor_zeta::send(current_message()->sender(), address(), handler_id(route::create_index_finish), session, cursor_t_ptr{new cursor_t(error_code_t::collection_dropped)});
        } else {
    ...
    void collection_t::create_index_finish(const session_id_t& session, const std::string& name, const actor_zeta::address_t& index_address) {
        ...
        actor_zeta::send(create_index.client, address(), handler_id(route::create_index_finish), session, name, cursor_t_ptr{new cursor_t()});
    ...
    void collection_t::drop_index(const session_id_t& session, components::ql::drop_index_t& index) {
        ...
        if (dropped_) {
            actor_zeta::send(current_message()->sender(), address(), handler_id(route::drop_index_finish), session, index.name(), cursor_t_ptr{new cursor_t(error_code_t::collection_dropped)});
        } else {
            ...
            if (index_ptr) {
                ...
                actor_zeta::send(current_message()->sender(), address(), handler_id(route::drop_index_finish), session, index.name(), cursor_t_ptr{new cursor_t()});
            } else {
                actor_zeta::send(current_message()->sender(), address(), handler_id(route::drop_index_finish), session, index.name(), cursor_t_ptr{new cursor_t(error_code_t::collection_not_exists)});
            }
        }
    }
    ...
    void collection_t::index_find_finish(const session_id_t& session, const std::pmr::vector<document_id_t>& result) {
        ...
        sessions::remove(sessions_, session);
        auto cursor = cursor_t_ptr{new cursor_t(context_->resource())};

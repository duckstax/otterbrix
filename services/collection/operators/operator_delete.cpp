...
namespace services::collection::operators {
    ...
    void operator_delete::on_execute_impl(components::transaction::context_t* transaction_context) {
        if (left_ && left_->output() && !left_->output()->documents().empty()) {
            ...
            for (const auto& document : left_->output()->documents()) {
                ...
                if (context_->storage().erase(context_->storage().find(id)) != context_->storage().end()) {
                    modified_->append(id);
                    context_->index_engine()->delete_document(document, transaction_context);
                }
            }
        }
    }

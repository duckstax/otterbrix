...
document_ptr gen_doc(const std::string& id,
                     const std::string& name,
                     const std::string& type,
                     ulong age,
                     bool male,
                     std::initializer_list<std::string> friends,
                     const document::retained_t<document::impl::mutable_dict_t>& sub_doc) {
    auto dict = document::impl::mutable_dict_t::new_dict();
    dict->set("_id", id);
    dict->set("name", name);
    dict->set("type", type);
...
context_ptr make_context(log_t& log) {
    ...
    context->resource = std::pmr::get_default_resource();
    context->manager_database_ =
        actor_zeta::spawn_supervisor<manager_database_t>(context->resource, context->scheduler_.get(), log);
    context->database_ =
        actor_zeta::spawn_supervisor<database_t>(context->manager_database_.get(), "TestDataBase", log, 1, 1000);
    //end comment
    ...
    auto* collection = new (buffer)
        collection_t(context->database_.get(), "TestCollection", log, actor_zeta::address_t::empty_address());
...
context_ptr gen_collection() {
    ...
    d(collection)
        ->insert_test(
            gen_doc("12345678123456789a00ff01", "Rex", "dog", 6, true, {"Lucy", "Charlie"}, gen_sub_doc("Lucy", true)));
    d(collection)
        ->insert_test(
            gen_doc("12345678123456789a00ff02", "Lucy", "dog", 2, false, {"Rex", "Charlie"}, gen_sub_doc("Rex", true)));
    d(collection)
        ->insert_test(
            gen_doc("12345678123456789a00ff03", "Chevi", "cat", 3, false, {"Isha"}, gen_sub_doc("Isha", true)));
    d(collection)
        ->insert_test(gen_doc("12345678123456789a00ff04",
                              "Charlie",
                              "dog",
                              2,
                              true,
                              {"Rex", "Lucy"},
                              gen_sub_doc("Lucy", false)));
    d(collection)
        ->insert_test(
            gen_doc("12345678123456789a00ff05", "Isha", "cat", 6, false, {"Chevi"}, gen_sub_doc("Chevi", true)));
...
TEST_CASE("collection_t find") {
    ...
    REQUIRE(res->size() == 1);
    res =
        d(collection)->find_test(parse_find_condition(R"({"$and": [{"age": {"$gt": 2}}, {"type": {"$eq": "cat"}}]})"));
    REQUIRE(res->size() == 2);
    res = d(collection)
              ->find_test(parse_find_condition(R"({"$or": [{"name": {"$eq": "Rex"}}, {"type": {"$eq": "cat"}}]})"));
    REQUIRE(res->size() == 3);
...
TEST_CASE("collection_t delete_one") {
    ...
    REQUIRE(d(collection)->size_test() == 5);
    REQUIRE(
        d(collection)->delete_one_test(parse_find_condition("{\"type\": { \"$eq\": \"dog\"}}")).deleted_ids().size() ==
        1);
    ...
    REQUIRE(
        d(collection)->delete_one_test(parse_find_condition("{\"type\": { \"$eq\": \"dog\"}}")).deleted_ids().empty());
    REQUIRE(d(collection)->size_test() == 2);
}
...
TEST_CASE("collection_t delete_many") {
    ...
    REQUIRE(d(collection)->size_test() == 5);
    REQUIRE(
        d(collection)->delete_many_test(parse_find_condition("{\"type\": { \"$eq\": \"dog\"}}")).deleted_ids().size() ==
        3);
    REQUIRE(d(collection)->size_test() == 2);
    REQUIRE(
        d(collection)->delete_many_test(parse_find_condition("{\"type\": { \"$eq\": \"cat\"}}")).deleted_ids().size() ==
        2);
...
TEST_CASE("collection_t update_one set") {
    ...
    auto result = d(collection)
                      ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                                        components::document::document_from_json(
                                            R"({"_id": "12345678123456789a00ff06", "$set": {"name": "Rex"}})"),
                                        false);
    ...
    result = d(collection)
                 ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                                   components::document::document_from_json(
                                       R"({"_id": "12345678123456789a00ff06", "$set": {"name": "Adolf"}})"),
                                   false);
    ...
    result = d(collection)
                 ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff06"}})"),
                                   components::document::document_from_json(
                                       R"({"_id": "12345678123456789a00ff06", "$set": {"name": "Rex"}})"),
                                   false);
    REQUIRE(result.modified_ids().empty());
...
TEST_CASE("collection_t update_many set") {
    ...
    auto result = d(collection)
                      ->update_many_test(parse_find_condition(R"({"type": { "$eq": "dog"}})"),
                                         components::document::document_from_json(
                                             R"({"_id": "12345678123456789a00ff06", "$set": {"name": "Rex"}})"),
                                         false);
    ...
    result = d(collection)
                 ->update_many_test(parse_find_condition(R"({"type": { "$eq": "mouse"}})"),
                                    components::document::document_from_json(
                                        R"({"_id": "12345678123456789a00ff06", "$set": {"name": "Mikki", "age": 2}})"),
                                    false);
    REQUIRE(result.modified_ids().empty());
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one inc") {
    ...
    auto result = d(collection)
                      ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                                        components::document::document_from_json(
                                            R"({"_id": "12345678123456789a00ff06", "$inc": {"age": 2}})"),
                                        false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set new field") {
    ...
    auto result = d(collection)
                      ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                                        components::document::document_from_json(
                                            R"({"_id": "12345678123456789a00ff06", "$set": {"weight": 8}})"),
                                        false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set complex dict") {
    auto collection = gen_collection();
    REQUIRE_FALSE(
        d(collection)->get_test("12345678123456789a00ff01").get_dict("sub_doc").get_dict("sub").get_string("name") ==
        "Adolf");
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff06", "$set": {"sub_doc.sub.name": "Adolf"}})"),
                              false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set complex array") {
    auto collection = gen_collection();
    REQUIRE_FALSE(
        d(collection)->get_test("12345678123456789a00ff01").get_array("friends").get_array(2).get_as<std::string>(0) ==
        "Adolf");
    auto result = d(collection)
                      ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                                        components::document::document_from_json(
                                            R"({"_id": "12345678123456789a00ff06", "$set": {"friends.2.0": "Adolf"}})"),
                                        false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set complex dict with append new field") {
    ...
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff06", "$set": {"new_dict.object.name": "NoName"}})"),
                              false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
    REQUIRE(d(collection)
                ->get_test("12345678123456789a00ff01")
                .get_dict("new_dict")
                .get_dict("object")
                .get_string("name") == "NoName");
}
...
TEST_CASE("collection_t update_one set complex array with append new field") {
    ...
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff06", "$set": {"new_array.1.5": "NoValue"}})"),
                              false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set complex dict with append new subfield") {
    ...
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff01"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff06", "$set": {"sub_doc.sub2.name": "NoName"}})"),
                              false);
    REQUIRE(result.modified_ids().size() == 1);
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE(result.upserted_id().is_null());
    REQUIRE(
        d(collection)->get_test("12345678123456789a00ff01").get_dict("sub_doc").get_dict("sub2").get_string("name") ==
        "NoName");
}
...
TEST_CASE("collection_t update_one set complex dict with upsert") {
    auto collection = gen_collection();
    REQUIRE(
        d(collection)->find_test(parse_find_condition("{\"new_dict.object.name\": {\"$eq\": \"NoName\"}}"))->empty());
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff10"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff10", "$set": {"new_dict.object.name": "NoName"}})"),
                              true);
    REQUIRE(result.modified_ids().empty());
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE_FALSE(result.upserted_id().is_null());
...
TEST_CASE("collection_t update_one set complex array with upsert") {
    ...
    auto result =
        d(collection)
            ->update_one_test(parse_find_condition(R"({"_id": { "$eq": "12345678123456789a00ff10"}})"),
                              components::document::document_from_json(
                                  R"({"_id": "12345678123456789a00ff10", "$set": {"new_array.0.0": "NoName"}})"),
                              true);
    REQUIRE(result.modified_ids().empty());
    REQUIRE(result.nomodified_ids().empty());
    REQUIRE_FALSE(result.upserted_id().is_null());

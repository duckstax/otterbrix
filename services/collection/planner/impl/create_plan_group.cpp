...
namespace services::collection::planner::impl {
            ...
            switch (expr->type()) {
                case scalar_type::get_field: {
                    ...
                    group->add_key(expr->key().as_string(),
                                   operators::get::simple_value_t::create(field));
                    break;
                }
                default:
    ...
    operators::operator_ptr create_plan_group(context_collection_t* context,
        ...
        std::for_each(node->expressions().begin(), node->expressions().end(),
                      [&](const components::expressions::expression_ptr& expr) {
            if (expr->group() == components::expressions::expression_group::scalar) {
                add_group_scalar(group, static_cast<const components::expressions::scalar_expression_t*>(expr.get()));
            } else if (expr->group() == components::expressions::expression_group::aggregate) {
                add_group_aggregate(context, group, static_cast<const components::expressions::aggregate_expression_t*>(expr.get()));
            }
        });
        return group;
    }

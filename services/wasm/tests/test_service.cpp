...
inline uint64_t test_handler_id(route type) {
    return 100 * 3773 + static_cast<uint64_t>(type);
}

class test_wasm_t final : public actor_zeta::cooperative_supervisor<test_wasm_t> {
public:
    test_wasm_t(pmr::memory_resource*);
...
test_wasm_t::test_wasm_t(pmr::memory_resource* resource)
...
auto test_wasm_t::make_scheduler() noexcept -> actor_zeta::scheduler_abstract_t* {
...
auto test_wasm_t::start(const std::filesystem::path& wasm_path) -> void {
    auto wasm = spawn_actor([this](wasm::wasm_runner_t* ptr) {
...
TEST_CASE("wasm_runner_t load and execute", "[API]") {
    ...
    pmr::memory_resource* resource = pmr::get_default_resource();

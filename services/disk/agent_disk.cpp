...
namespace services::disk {
    ...
    agent_disk_t::agent_disk_t(manager_disk_t* manager, const path_t& path_db, log_t& log)
        ...
        , append_database_(actor_zeta::make_behavior(resource(), handler_id(route::append_database),this, &agent_disk_t::append_database))
        , remove_database_(actor_zeta::make_behavior(resource(), handler_id(route::remove_database),this, &agent_disk_t::remove_database))
        , append_collection_(actor_zeta::make_behavior(resource(), handler_id(route::append_collection),this, &agent_disk_t::append_collection))
        , remove_collection_(actor_zeta::make_behavior(resource(), handler_id(route::remove_collection),this, &agent_disk_t::remove_collection))
        , write_documents_(actor_zeta::make_behavior(resource(), handler_id(route::write_documents),this, &agent_disk_t::write_documents))
        , remove_documents_(actor_zeta::make_behavior(resource(), handler_id(route::remove_documents),this, &agent_disk_t::remove_documents))
        , fix_wal_id_(actor_zeta::make_behavior(resource(), handler_id(route::fix_wal_id),this, &agent_disk_t::fix_wal_id))
        , log_(log.clone())
        , disk_(path_db) {
        trace(log_, "agent_disk::create");
    }

    auto agent_disk_t::make_type() const noexcept -> const char* const{
        return "agent_disk";
    }

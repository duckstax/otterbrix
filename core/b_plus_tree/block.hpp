...
namespace core::b_plus_tree {
    ...
    public:
        ...
        public:
            ...
            inline const iterator& operator++() { metadata_--; rebuild_data(); return *this; }
            inline const iterator& operator--() { metadata_++; rebuild_data(); return *this; }
            inline iterator operator++(int) { auto temp = iterator(block_, metadata_); metadata_--; rebuild_data(); return temp; }
            inline iterator operator--(int) { auto temp = iterator(block_, metadata_); metadata_++; rebuild_data(); return temp; }
            inline iterator operator+(int i) { return iterator(block_, metadata_ - i); }
            inline iterator operator-(int i) { return iterator(block_, metadata_ + i); }
            friend inline iterator operator+(int i, const iterator& rhs) { return iterator(rhs.block_, rhs.metadata_ - i); }
            friend inline iterator operator-(int i, const iterator& rhs) { return iterator(rhs.block_, rhs.metadata_ + i); }
            friend int operator-(const iterator& lhs, const iterator& rhs) { assert(lhs.block_ == rhs.block_); return rhs.metadata_ - lhs.metadata_; }

            inline iterator& operator=(const iterator& rhs) { metadata_ = rhs.metadata_; rebuild_data(); return *this; }
            inline iterator& operator+=(int rhs) { metadata_ -= rhs; rebuild_data(); return *this; }
            inline iterator& operator-=(int rhs) { metadata_ += rhs; rebuild_data(); return *this; }
        ...
        public:
            ...
            // note: metadata is stored in reverse order, so iterator increment and decrement is reversed
            inline const r_iterator& operator++() { metadata_++; rebuild_data(); return *this; }
            inline const r_iterator& operator--() { metadata_--; rebuild_data(); return *this; }
            inline r_iterator operator++(int) { auto temp = r_iterator(block_, metadata_); metadata_++; rebuild_data(); return temp; }
            inline r_iterator operator--(int) { auto temp = r_iterator(block_, metadata_); metadata_--; rebuild_data(); return temp; }
            inline r_iterator operator+(int i) { return r_iterator(block_, metadata_ + i); }
            inline r_iterator operator-(int i) { return r_iterator(block_, metadata_ - i); }
            friend inline r_iterator operator+(int i, const r_iterator& rhs) { return r_iterator(rhs.block_, rhs.metadata_ + i); }
            friend inline r_iterator operator-(int i, const r_iterator& rhs) { return r_iterator(rhs.block_, rhs.metadata_ - i); }
            friend int operator-(const r_iterator& lhs, const r_iterator& rhs) { assert(lhs.block_ == rhs.block_); return lhs.metadata_ - rhs.metadata_; }

            inline r_iterator& operator=(const r_iterator& rhs) { metadata_ = rhs.metadata_; rebuild_data(); return *this; }
            inline r_iterator& operator+=(int rhs) { metadata_ += rhs; rebuild_data(); return *this; }
            inline r_iterator& operator-=(int rhs) { metadata_ -= rhs; rebuild_data(); return *this; }
        ...
        [[nodiscard]] std::pair<std::unique_ptr<block_t>, std::unique_ptr<block_t>> split_append(uint64_t id, const_data_ptr_t append_buffer, size_t buffer_size);
    ...
    [[nodiscard]] static inline std::unique_ptr<block_t> create_initialize(std::pmr::memory_resource* resource, size_t size = DEFAULT_BLOCK_SIZE) {
